...
export const FieldListViewData = () => {
  ...
  const [triggerGetModels, { data: field, isLoading, error }] =
  ...
  useEffect(() => {
    if (field) {
      ...
      setTotalRecords(field?.meta.totalRecords);
      setLoading(false);
    }
  }, [field]);
  ...
  const detailsBodyTemplate = (fieldMetaData: ModelMetaData) => {
    return (
      <Link
        href={fieldMetaData.id}
    ...
    <div className="">
      ...
      >
        ...
        <Column
          ...
          // sortable
          filter
          filterPlaceholder="Search Fields by Model name"
          style={{ minWidth: "12rem" }}
          headerClassName="table-header-fs"
        ></Column>
