...
var SolidInitialLoginOtp = function () {
    ...
    var tempIdentifier = searchParams.get('identifier');
    var type = searchParams.get('type') || 'email';
    var identifier = tempIdentifier ? decodeURIComponent(tempIdentifier) : '';
    var RESEND_OTP_KEY = "resendOtpLogin_".concat(identifier);
    var RESEND_OTP_TIMER_MIN = parseFloat(env("NEXT_PUBLIC_RESEND_OTP_TIMER") || '0.5');
    var RESEND_OTP_TIMER = Math.round(RESEND_OTP_TIMER_MIN * 60);
    var _f = useLazyGetAuthSettingsQuery(), trigger = _f[0], solidSettingsData = _f[1].data;
    useEffect(function () {
    ...
    useEffect(function () {
        ...
        var storedTime = localStorage.getItem(RESEND_OTP_KEY);
        if (!storedTime) {
            localStorage.setItem(RESEND_OTP_KEY, Date.now().toString());
        }
        var lastSent = storedTime ? parseInt(storedTime, 10) : Date.now();
        var elapsed = Math.floor((Date.now() - lastSent) / 1000);
        var remaining = RESEND_OTP_TIMER - elapsed;
        if (remaining > 0) {
        ...
        return __generator(this, function (_c) {
            switch (_c.label) {
                case 0:
                    ...
                    return [4 /*yield*/, initiateResendOTP(payload).unwrap()];
                case 1:
                    response = _c.sent();
