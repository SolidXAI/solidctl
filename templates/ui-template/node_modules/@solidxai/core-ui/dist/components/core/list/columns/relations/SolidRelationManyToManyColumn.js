...
var SolidRelationManyToManyColumn = function (_a) {
    ...
    return (_jsx(Column, { field: fieldMetadata.name, header: header, 
        ...
        }, filterPlaceholder: "Search by ".concat(fieldMetadata.displayName), style: { minWidth: "12rem" }, headerClassName: "table-header-fs" }, fieldMetadata.name));
};
export default SolidRelationManyToManyColumn;
export var DefaultRelationManyToManyListWidget = function (_a) {
    var _b, _c, _d, _e;
    var rowData = _a.rowData, solidListViewMetaData = _a.solidListViewMetaData, fieldMetadata = _a.fieldMetadata, column = _a.column;
    var manyToManyFieldData = rowData[column.attrs.name];
    ...
    if (manyToManyFieldData) {
        ...
        var manyToManyColVal = manyToManyFieldData.map(function (f) { return f[userKeyField_1]; });
        // TODO: change this to use an anchor tag so that on click we open that entity form view. 
        return (_jsx(_Fragment, { children: manyToManyColVal.length > 0 &&
                _jsxs("p", { children: [_jsx("span", { style: { backgroundColor: '#f5f5f5', padding: '7px', borderRadius: '4px' }, children: manyToManyColVal[0] }), _jsx("span", { style: { color: "#0895CD", fontWeight: 'bold' }, children: manyToManyColVal.length - 1 > 0 ? " +".concat(manyToManyColVal.length - 1) : "" })] }) }));
    }
    else {
        return _jsx("span", {});
    }
};
//# sourceMappingURL=SolidRelationManyToManyColumn.js.map